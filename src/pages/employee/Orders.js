...
export default function OrdersEmployee() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/v1/order?page=${currentPage}`
        );
  ...
  const handleUpdate = (order) => {
    navigate(`/employee/orders/update/${order.id}`);
  };
  ...
  return (
    <div className="order-container">
      ...
      <div className="borderFilterMobile option browse-tags">
        <label className="lb-filter" htmlFor="status-filter">
          Filter by status:
        </label>
        ...
        <tbody>
          {filteredData.map((order) => (
            <tr className="order-item" key={order.id}>
              <td className="order-item-address">
