...
const NewProduct = () => {
    ...
    // Chuyển đổi tag: thay thế '-' bằng ' ' và viết hoa chữ cái đầu của mỗi từ
    const taglist = tag.replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [taglist]); // Thêm taglist vào dependency array
