...
const OrderUpdate = () => {
    ...
    const fetchOrderItems = async () => {
      try {
        const itemsResponse = await axios.get(
          `http://localhost:3001/api/v1/orderItem/order/${id}`
        );
  ...
  const handleUpdate = async (event) => {
    ...
    if (orderDetails.status === "completed" && orderStatus === "cancelled") {
      alert(
        "Không thể cập nhật trạng thái đơn hàng từ 'completed' sang 'cancelled'!"
      );
      return;
    }
    if (orderDetails.status === "cancelled") {
      alert("Không thể cập nhật đơn hàng đã bị hủy.");
      return;
    }
    const updatedData = {
      status: orderStatus,
    ...
    <div className="view-container">
      ...
      {orderDetails && (
        <form onSubmit={handleUpdate}>
          <p>
            <strong>Tên:</strong>{" "}
            <i>{orderDetails.user ? orderDetails.user.name : "N/A"}</i>
          </p>
          <p>
          ...
          <p>
            <strong>Status:</strong> <i>{orderDetails.status}</i>
          </p>
          <p>
          ...
          <p>
            ...
            <i>
              {moment(orderDetails.updated_at).format("YYYY-MM-DD HH:mm:ss")}
            </i>
          </p>
          <div>
            <p className="stt-book">
          ...
          <button className="sub-update" type="submit">
            Update
          </button>
        </form>
      )}
