  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const totalRevenueInCurrentMonth = revenueInCurrentMonth.reduce(
          (total, order) => total + parseFloat(order.total_amount),
          0
        );
        const totalRevenueInPreviousMonth = revenueInPreviousMonth.reduce(
          (total, order) => total + parseFloat(order.total_amount),
          0
        );
        ...
        if (totalRevenueInPreviousMonth !== 0) {
          percentIncrease =
            ((totalRevenueInCurrentMonth - totalRevenueInPreviousMonth) /
              totalRevenueInPreviousMonth) *
            100;
  ...
  return (
    <div className="grid-cart-revenue-container" container spacing={3}>
      <div className="grid-cart-revenue-box" style={{ marginBottom: "20px" }}>
        ...
        <h3>{formatPrice(totalRevenue.toFixed(2))}</h3>{" "}
